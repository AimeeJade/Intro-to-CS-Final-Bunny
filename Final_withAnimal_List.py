...
def main():
    ...
    win = GraphWin("Bunny", 700, 700, autoflush = False)
...
#--------------------------Creating Animal Instances at Random Points----------------------------
    ...
    #-------- When the mouse click falls into a certain range in the animal's head, the object is undrawn --------
        ...
        if clickPoint == 'g':
            ...
            grass_list.append(grass)
            grass_top_list.append(grass.grasstop)
        #updating animal_list
        animal_list = bunny_list + fox_list + eagle_list
        update(35)
    #-------- Animal movements --------
        for animal in animal_list:
            animal.move(animal.slope)
...
class Bunny:
   def __init__(self, bun_x, bun_y, right):
       ...
       self.slope = (18 * pi * cos((3 * pi * self.x) / 1000) * sin((3 * pi * self.x) / 1000))/(5 * abs(sin((3 * pi * self.x) / 1000)))
   ...
   def move(self, slope):
       ...
       if self.x > 1000: 
            self.x -= 3
            self.slope = (18 * pi * cos((3 * pi * self.x) / 1000) * sin((3 * pi * self.x) / 1000))/(5 * abs(sin((3 * pi * self.x) / 1000)))
            self.bunear1.move(-3, -self.slope)
            self.bunear2.move(-3, -self.slope)
            self.bunbody.move(-3, -self.slope)
            self.bun.move(-3, -self.slope)
            self.buntail.move(-3, -self.slope)
            self.right = False
...
class Fox:
   ...
   def move(self, slope):
       self.fox_slope = slope
       #move left
       if self.x > 1000:
            self.x -= 10
            self.slope = 0
            self.fox.move(-15, self.slope)
            self.foxbody.move(-15, self.slope)
            self.foxear1.move(-15, self.slope)
            self.foxear2.move(-15, self.slope)
            self.foxtail.move(-15, self.slope)
            self.foxnose.move(-15, self.slope)
            self.right2 = False
       #move right
       elif self.x < 0:
            self.x += 10
...
class Eagle:
   def __init__(self, eagle_x, eagle_y, right3):
       ...
       self.slope = (150 * cos((self.x/90) + 18))/9
   ...
   def move(self, slope):
       ...
       if self.x > 1000:
            self.x -= 5
            self.slope = (150 * cos((self.x/90) + 18))/9
            self.eagle.move(-5,-self.slope)
            self.eaglebody.move(-5,-self.slope)
            self.eaglewing1.move(-5,-self.slope)
            self.eaglewing2.move(-5,-self.slope)
            self.eaglebeak.move(-5,-self.slope)
            self.right3 = False
        #move right
       elif self.x < 0:
            self.x += 5
