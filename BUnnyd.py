...
def main():
    ...
    win = GraphWin("Bunny", 1000, 500, autoflush=False)
...
class Bunny:
    ...
    def move1(self):
        self.bun_slope = (18 * pi * cos((3 * pi * self.x) / 1000) * sin(
                (3 * pi * self.x) / 1000))/(5 * abs(sin((3 * pi * self.x) / 1000)))
...
class Fox:
    def __init__(self, fox_x, fox_y, right2):
        ...
        self.foxear1 = Polygon(Point(self.x-50, self.y + 20),
                               Point(self.x-90, self.y+70), Point(self.x - 20, self.y+50))
        self.foxear1.setFill("coral")
        self.foxear2 = Polygon(Point(self.x+50, self.y + 20),
                               Point(self.x+90, self.y+70), Point(self.x + 20, self.y+50))
        self.foxear2.setFill("coral")
        self.foxtail = Polygon(Point(self.x + 50, self.y - 90), Point(
            self.x + 150, self.y - 30), Point(self.x + 90, self.y - 100))
        self.foxtail.setFill("coral")
        self.foxnose = Polygon(Point(
            self.x, self.y - 10), Point(self.x + 10, self.y), Point(self.x - 10, self.y))
